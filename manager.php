...
<html>
<head><title>HealthFog - Manager (Master)</title>
</head>
<body>
...

{
	...
	// Alter first line of config.txt as per Enable master or Cloud set or not
	if(isset($_POST['enableMaster'])){
		if(isset($_POST['enableCloud'])){
			file_put_contents("config.txt", "EnableMaster EnableCloud".PHP_EOL.$content);
		}
		else{
			file_put_contents("config.txt", "EnableMaster DisableCloud".PHP_EOL.$content);
		}
	}
	else{
		if(isset($_POST['enableCloud'])){
			file_put_contents("config.txt", "DisableMaster EnableCloud".PHP_EOL.$content);
		}
		else{
			file_put_contents("config.txt", "DisableMaster DisableCloud".PHP_EOL.$content);
		}
	}
	...
	<input type='checkbox' name='enableCloud' value='Yes' checked />Enable Cloud<br/>
...
if(isset($_POST['remove'])){
	file_put_contents("config.txt", "EnableMaster EnableCloud".PHP_EOL);
